...
// Boolean value set to 1 if text displayed on screen, 0 otherwise.

static int textdisplayed = 1;

// Buffer used to store text from screen when we are in a graphics mode

static struct {
    ushort  videobuffer[80 * 25];
    int     cursorposition;
} consbuffer;;
...
void grabscreentobuffer() {
	...
	consbuffer.cursorposition = gethardwarecursorposition();
    textdisplayed = 0;
}
